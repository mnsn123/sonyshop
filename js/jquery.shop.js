		...
		displayCart: function() {
			if( this.$formCart.length ) {
				...
				var html = "<tr><td class='pname'>" + product + "</td>";
						html += "<td class='pqty qid'>" +
							  "<span class='value-button minus' onclick='decreaseValue2(this)'>-</span>" +
							  "<input type='number' class='qty number' value='" + qty + "' data-product='" + product + "' />" +
							  "<span class='value-button plus' onclick='increaseValue2(this)'>+</span></td>";
						html += "<td class='pprice'>" + price + "</td>" +
							        "<td class='pdelete'><a href='' data-product='" + product + "'><i class='fa fa-times' aria-hidden='true'></i></a></td></tr>";
			...
			if( this.$checkoutCart.length && this.$userDetails.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						email_content += encodeURI('Order Received' + '\n' + '----------------' + '\n' + ' Item - ' + product + '\n' + 'Price - ' + this.currency + ' ' + price + '\n' + 'Quantity - ' + qty + '\n' + 'Sub Total - ' + this.currency + ' ' + parseFloat(etotal).toFixed(2) + '\n' + '-----------------' + '\n');
					
						// $tableCartBody.html( $tableCartBody.html() + html );
					}
					
					email_content += encodeURI('*****************' + '\n' + 'Grand Total - ' + this.currency + ' ' + parseFloat(this.storage.getItem( this.total )).toFixed(2) + '\n\n');
					if( this.storage.getItem( "shipping-name" ) == null ) {
						var name = this.storage.getItem( "billing-name" );
						var email = this.storage.getItem( "billing-email" );
						var city = this.storage.getItem( "billing-city" );
...
//																													 $tableCartBody.html( $tableCartBody.html() + html );
																						...
																						var p = document.querySelector('.message_checkout');

																					    var cmobile = '{{ site.mobile }}';
																					    p.href = 'sms:'+ cmobile +'?body='+email_content;
																					}
